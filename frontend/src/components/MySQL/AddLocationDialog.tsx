...
const AddLocationDialog = ({ onSuccess }: AddLocationDialogProps) => {
  ...
  const onSubmit = async (data: LocationFormData) => {
    setLoading(true)
    try {
      const response = await fetch("/api/v1/mysql/locations", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`创建位置上报失败: ${errorData.detail || "未知错误"}`)
      }
    } catch (error) {
      console.error("创建位置上报失败:", error)
      alert("创建位置上报失败，请检查网络连接")
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>新增位置上报</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <Text mb={4}>填写以下信息添加新的位置上报记录</Text>
            <VStack gap={4}>
              <HStack w="full" gap={4}>
                <Field
                  label="用户ID"
